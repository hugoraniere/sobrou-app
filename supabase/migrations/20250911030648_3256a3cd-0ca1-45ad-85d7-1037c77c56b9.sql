...
CREATE OR REPLACE FUNCTION public.get_public_blog_post(post_id UUID)
RETURNS TABLE (
  id UUID,
  title TEXT,
  subtitle TEXT,
...
BEGIN
  ...
  IF user_id_param IS NOT NULL THEN
    SELECT EXISTS(
      ...
      WHERE post_id = $1 AND user_id = user_id_param
    ) INTO like_exists;
    ...
    ELSE
      ...
      VALUES ($1, user_id_param);
      RETURN TRUE;
    END IF;
  ELSE
    SELECT EXISTS(
      SELECT 1 FROM public.blog_post_likes 
      WHERE post_id = $1 AND ip_address = ip_address_param
    ) INTO like_exists;
