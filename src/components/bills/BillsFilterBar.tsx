...
export const BillsFilterBar: React.FC<BillsFilterBarProps> = ({
  ...
  return <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {periodOptions.map(option => <Button key={option.value} variant="outline" size="sm" onClick={() => onPeriodFilterChange(option.value as BillsPeriodFilter)} className={cn("h-8 px-3 text-xs", periodFilter === option.value ? "bg-primary text-primary-foreground border-primary hover:bg-primary/90" : "bg-background hover:bg-muted")}>
            <Calendar className="h-3 w-3 mr-1.5" />
            {option.label}
          </Button>)}
        
        {/* Dropdown de mês customizado */}
        {periodFilter === 'custom-month' && <Select value={customMonth} onValueChange={onCustomMonthChange}>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar conta por nome..." value={searchTerm} onChange={e => onSearchChange(e.target.value)} className="pl-10 bg-neutral-50" />
        </div>

        {/* Toggle de contas pagas */}
        <div className="flex items-center space-x-2 whitespace-nowrap">
          <Switch id="hide-paid" checked={!hidePaid} onCheckedChange={checked => onHidePaidChange(!checked)} />
